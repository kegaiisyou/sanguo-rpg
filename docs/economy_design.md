...
> **状态（部分落地 · 与代码已有出入）**：本文是早期经济系统**设计草案**，提出「货币分层 / 材料字典 / 容器模型 / 配方引擎」骨架。其中：
> - ✅ **已落地**：物资合成（炊事灶：金疮药/肉包子，v20260821j）、城市营造 MVP（蓝图→工地→填充→搭建→落成，含冶炼工坊炉膛烧铁→铁器打造）、货郎基础买卖。
> - ⚠️ **与代码分歧**：草案假设的货币字段 `state.coin`、材料字典 `LF.MATERIALS`、`state.cart/stocks/veins` 等**未原样采用**——当前实际货币为「银两」，材料/物品统一走 `shared/data/items.js` 的 `DEFS`（`cat` 分类 + 堆叠），背包即 `state.pack`。请勿把本文当代码现状，落地时以 `PROGRESS.md` 与 `shared/data/*` 为准。
> 本文保留作经济系统**扩展示例与接口思路**（黑市/商路/物价波动/多容器物流等仍待做）。
