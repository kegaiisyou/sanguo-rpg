...
| **标题屏 UI** | ✅ 完成 | 长江落日水墨标题屏：四入口（仗剑入世/拾卷续缘/览物志/整肃）；三档存档（lf_save_1~3，旧档 lf_save_v1 自动迁第一档）；死亡弹「殒落」回标题屏（可拾卷续缘或读档续命）；图鉴（武学/门派/贼寇名录）与设置（演出速度/自动战斗）弹窗 | `index.html`（#title / openModal newgame·load·codex·settings / enterGame·showTitle / saveToSlot·slotMeta）、`shared/index.js` |
...
### 6.3 主线流程
颍川主营（出生·研习武学） → 剿匪/巡山攒声望 → 声望≥20 → 挑战台·温酒斩华雄 → 解锁洛阳 → 赴洛阳凯旋
...
*最后更新：2026-07-24（**标题屏与三档存档**：① 新增长江落日水墨标题屏（#title），四入口「仗剑入世/拾卷续缘/览物志/整肃」，文言好懂、切合三国武侠；② 存档由单键改为三档（lf_save_1~3），旧档 lf_save_v1 启动自动迁第一档，读档/开新局均经择档面板；③ 死亡弹「殒落」提示并回标题屏（可拾卷续缘或读档续命）；④ 图鉴「览物志」（武学十三艺线/门派/贼寇名录，已历标朱）与设置「整肃行装」（演出速度疾/常/缓、默认自动战斗）弹窗；历法/天候/NPC菜单/光和元年/清理测试实体/街道拓扑/正北太行/巡检修复 已完成）*
