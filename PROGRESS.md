...
| 设计项 | 状态 | 实际落地 | 代码落点 |
|--------|------|----------|----------|
| **苦役营体验修复** | ✅ v20260917e | 实机体验修复 4 项：①教学目标方向词与高亮统一（「往南回牢区」→「往北」，方向一律按 `campDirTo` 现算不再写死）；②应卯唱名者「校尉」→「牢头」（营规掌者是牢头，叙事一致）；③新档查记工册不再误显「已脱籍」（区分未录名/已脱籍两种文案）；④叩门开锁时牢头顺口预告「戌时前回牢过夜」 | `engine.onbGoalStep/campGoto`、`engine.doRollCall`、`engine.ledgerLook`、`triggers.laotou_door` |
...
### §9.45 v20260905h–i：出城/回城闭环改造（`de9b4e9`）

**背景**：§9.44 后「出城入野」已通，但链路仍有断裂与歧义：① `travel.js link()` 读宿主房间表写的是 `global.G`，而 index.html 里房间表是 `window.LF.SharedGame` 的局部 `var G`（从未暴露 `window.G`）→ 数据作用域内取不到 → link() 早退，`__gate__:` 哨兵出口（入口格回母城、远边格入邻城）从未接线，**出得去进不来**；② 出城交互入口混乱（城门场景「出城门」按钮 + 「寻路出城」自动代出城，语义不清）。

**改动**
- `travel.js link()` 取 G 修复：`var G = global.G || (global.LF && (global.LF.SharedGame || global.LF));`——浏览器真机与 Node 校验都能取到房间表，哨兵出口恢复接线（城↔郊野↔城双向可走）。
- **出城统一走移动罗盘（h）**：删除城门格「出城门」场景按钮与「寻路出城」（旧版会自动代出城，一步跨过城门语义）。立于城门格时罗盘自动出现**朝外「出城」方向**（新 `.mv-exit.gateout` 暖金晕边样式 + `title` 悬停显示去向）；城内任意格按钮改为「🚪 前往城门」（`leave_auto`）——沿可通行道路寻路**只抵最近可出城城门**（死门过滤：`gateOutwardDir` 有郊野通途才算，避免送到无路的门形墙格），不再代出城，log 引导「城门在望——看罗盘，朝城外方向踏出即离城」。
- **i 微调**：罗盘钮名由「东·出城(某郊野全名)」缩为「东·出城」（去向写入 `title`，防 3×3 窄钮截断长名）；`leave_city` 兼容入口语义分层——立于可出城门口则真出城（布防图打开时先收起露出郊野主界面），其余位置直接打开世界山河志。

**校验**：`_repro_exit.js`（jsdom 整页：读档蓟城 → 城门格罗盘 gateout 出城 → 郊野入口格 `__gate__` 回城落城门格 → `leave_city` 兼容路径）全 PASS；`_verify_crosscity.js` / `_verify_travel.js` 拓扑回归 OK。版本 **20260905g→20260905i**（constants.js + index.html `?v` 同步）。

### §9.46 v20260905j：跨城步行入城修复 + 城内地图双页签（`6043a3c`）

**背景**：逐格实测发现「洛阳东郊野→河内」入城哨兵丢失——`lvboshe` 与河内同占洛阳东野远边格，`link()` 逐个邻点「后写覆盖先写」把前者的入城哨兵冲掉 → 穿野到不了河内。另有城门分组按「四方向量化→就近补门」在门角平分/tie 处把邻点塞进背向门郊野 → `along<=0` 跳过 → 单向断头。

**改动（travel.js）**
- **分组按「真朝外的门」归组**：新增 `pickOutwardGate(gdirs, v)`——候选门只取「轴向分量 >0（真能朝外通到邻点）」中夹角最近者；若无任何开门朝向该邻点（如单门山城、邻点在背侧）退回最近门（物理不可达属合理）。旧版量化 tie 造成的背向分派消除。
- **远边出野口独占分配**：外向邻点先按横向投影排序（稳定可复现），再逐格独占落槽、冲突向外就近寻空槽——每个外向邻点都有**唯一**出野口，不再互相覆盖。已建成郊野不重建，需清缓存刷新。
- **`arriveAtGate` 落点兜底**：目标城指定侧可能没有真城门（单门山城只朝固定方向开门），落墙/屋格会困死玩家；新增 `nearestGateCell`——指定门洞非「可进入 gate 格」时就近落到最近的真正城门格。

**改动（UI，index.html + strategic-map.js）**
- **城内地图双页签**：城内开「地图」→ `buildCityMapTabsHTML()`（🏯 城内布防图 ↔ 🗺 山河志·十三州 页签互切），移除旧 `mc-back`「返回山河志（出城）」按钮；山河志页首次切到才初始化，避免打开即加载 D3 战略图。
- **山河志打开默认居中「此身所在」**：`strategic-map.js` 的 `locateGuide(instant)` 支持跳过补间，init 后下一帧 `locateGuide(true)` 居中玩家（k≈2.6 中近视野），不再永远首览十三州全景；world 页经 `opts.focusYou` 透传。

**校验**：`_verify_crosscity.js` 跨城哨兵 193 对全可达、断头归零前的结构性回归；jsdom 整页洛阳→河内/弘农入城落 gate 格 OK；版本 **i→j**（constants.js + index.html + travel.js + strategic-map.js `?v` 同步）。

### §9.47 v20260905k：城门方向路网自适应（`ecd0ba1`）

**背景**：此前门向按 `ctype` 固定方向序取前 nG（plain 北东南西 / mountain 北西南东 / fort 北南西东 / port 北东南西）——山城只开 2 门却可能把门开在**没有路的山侧**，邻点在南/东就永远出不去。审计出孤立/软锁网格城：晋阳/武威/上庸 **out=0**（有邻点但无门朝外）、交趾 **in=0**（邻城门都不朝它）→ 跨城断头 39。

**改动（index.html）**
- 新增 `cityGateDirs(cid)`（带 `CITY_GATE_DIRS_CACHE` 缓存）：枚举全部 C(4, nG) 开门组合（16 掩码筛 nG 位），评分 = 开门能把路网邻点**正面分组**（`pick` 轴向>0）的个数（网格城邻 ×2 / 野外地点邻 ×1，city 邻权重更高因为哨兵是唯一入口），同分取 **ctype 缺省方位序亲和**（`4-ord.indexOf(d)` 累加）——已有最优布局的城市选择不变。
- **门洞格 / 进城落点 / 寻路目标 / 罗盘全部同源**：`genCityGrid` 门洞格改由 `cityGateDirs` 生成（删旧 `gateOrder` 硬编码）、`availableGateDirs` 直接返回 `cityGateDirs`、进城落点按开着的门依次取可通行格（兜底城心）；grid cache ver bump。
- 效果：所有孤立/软锁网格城消除，跨城断头 39→16（剩 16 为门向选择问题，见 §9.48）。版本 **j→k**（constants.js + index.html `?v` 同步）。

### §9.48 v20260905l：全图强连通（`df5c4cc`）

**背景**：k 后剩 16 条跨城断头——61 城步行有向图呈 **4 个强连通盆**（中原 / 凉州 / 蜀 / 岭南），盆与盆在 ROADS 上本有跨盆邻接（如 42↔凉↔蜀↔岭南↔42 成环）但盆缘城开门没朝对方向。束搜索 + 离线原型证明：**只有让梓潼开出朝汉中的门**（补蜀→凉这条唯一缺失的环边）才能把 4 盆并入一个 SCC。

**改动**
- `shared/data/cities.js`：梓潼 `wall 50→55`——山城按 wall 决定开 2 门，提墙后开门组合由「北+西」翻成「**北+南**」：南门保成都走廊（蜀内）、北门朝汉中/武都接凉州（蜀→凉），**一扇门同时满足两个盆的诉求**（代价权衡：北侧陆路不再有西向广元一侧，但蜀内仍有其他连法兜底）。
- `shared/systems/travel.js`：网格城郊野分组**剔除后向成员**——按 `pickOutwardGate` 分到某开门、但 link() 远边 `along<=0` 物理连不上的邻点（真相是城没开朝它的门）不再进 `meta.neighbors`；城内罗盘/行军列表不再显示永远走不通的死目标（此前这类成员正是 16 条断头的来源）。
- 结果：城↔城直连对 208→193（剔除物理不可达项）、**断头 16→0**；61 城有向步行图**强连通（SCC=1）**，无孤立/软锁城；洛阳→河内/弘农等关键入城路径保持可走；grid cache ver bump。

**校验（全绿）**：`_verify_crosscity.js` 193/193 哨兵可达 + 强连通 + 无出入空缺网格城；`_verify_travel.js` 全 61 城可达、无死胡同、无缺失反向出口；`_repro_exit.js`（蓟城）T1–T7 PASS；`_verify_field.js`/`_verify_bands.js`（3136 格内容契约 / band 分布 4×784）无回归。版本 **k→l**（constants.js + index.html 的 constants/cities/travel `?v` 同步；存档 schema `0.2.0` 未动）。

---

### §9.49 v20260905m：清理一次性脚本与死引用（P0）

**改动**
- 移除 `index.html` 中 `turf.min.js`（约 590KB）的 `<script>` 引用——jsdom 战略图冒烟确认 turf 运行时零调用，删除后渲染仍 209 path + 61 城/特殊节点、0 报错。
- `.gitignore` 补 `_*.js`：把 `_verify_*/_repro_*/_smoke_*` 临时校验脚本纳入忽略（文件保留本地，待 P1-4 收编进 `test/`）。
- `patch_index.js` / `patch_map.js`（含硬编码绝对路径的一次性改写工具）`git mv` 至 `_archive/`。
- `gen_cities.js` 文件头加「一次性引导脚本·切勿重跑」警告，防覆盖梓潼 wall 55、owner 双源对齐等人工修复。
- 版本 **l→m**（constants.js + index.html `constants.js?v=` 同步）；其它文件未改，其 `?v` 保持原值。

### §9.50 v20260905n：野外地图双页签

**背景**：§9.46 已为城内做「🏯 城内布防图 ↔ 🗺 山河志·十三州」双页签；但野外（郊野）打开地图仍只直出十三州战略图，缺一个类似城里「布防图」的**局部郊野视图**。

**改动（index.html）**
- 新增 `buildFieldMapHTML()` / `buildFieldMapTabsHTML()`：野外开「地图」→ `🏕 郊野图 ↔ 🗺 山河志·十三州` 双页签。郊野图渲染当前郊野 4×4 局部网格（金框＝你的所在、🚪＝入口回母城，标注 🌿资源 / 🐺野兽 / 💬路人，底部列出「远边行军 → 邻城」）；十三州页复用 D3 战略图并 `focusYou` 居中你所在的郊野插值位置（沿用 `smYouMark` 郊野分支）。
- `openModal('map')` 分支与绑定逻辑：城内→城内双页签、野外(`isField`)→野外双页签、其余→战略图；页签绑定复用 `initMapTabs()`（十三州页懒加载）。
- 版本 **m→n**（constants.js + index.html `constants.js?v=` 同步）。

**校验（全绿）**：`_smoke_field_map.js` 野外页签✓ 16 格✓ 邻城出口✓ 切十三州 209 path/0 报错✓；城内冒烟、HTML 语法 0 错、交叉引用 0 硬错、战斗 45/45、营造全过均无回归。

---

### §9.51 v20260905o：郊野〔途〕提示与罗盘一致化

**背景**：§9.45 起的郊野〔途〕提示用「整片郊野的 gateDir + meta.neighbors」播报「向X（出野）可至 邻城」，但出野出口（`travel.js:310 cell.exits[dir]=tgt`）只写在远野边**特定格**、每格仅通一个邻城。玩家立于无出口的远野格时，提示却说「向北出野可至寿春」、罗盘却无北按钮——误导（实测 庐江北郊野 4 个远野格仅 c=1 有 北→寿春，余 3 格无出口）。全量扫描：158 个有邻城郊野中 94 个存在「部分远野格无出野按钮」。

**改动（index.html `fieldNarr`）**：〔途〕提示改为**基于当前格实际 `exits`**——优先列出本格真实出野出口（方向+邻城，与罗盘完全对应）；当前格无出野出口时改为引导「向<gateDir>深入至远野边格可出野（可至 邻城列表）」，不再谎称「向北出野」。排除回母城哨兵（`__gate__:<母城>`）以免把回城方向误列为出野。

**校验（全绿）**：`_diag_field_exit.js` 直接调用 `fieldNarr`——`fld_lujiang_北@0_0`（无出口）：「…向北深入至远野边格可出野（可至 寿春）」；`fld_lujiang_北@0_1`（有 北→寿春）：「…此格向北至寿春（出野）」——与罗盘一一对应；`_verify_crosscity.js`/`_verify_travel.js` 拓扑回归 pageErrors=0、193/193、全连通无回归。版本 **n→o**（constants.js + index.html `constants.js?v=` 同步）。
...
## §9.68 战略系统实机测试 · 修复「前往此城」断线（v20260918a）

**背景**：用户要求测试战略系统（dock「🗺️ 山河」→ 山河志）。Playwright 实机走完教学链后逐项验证，发现并修复一处遗留断线。

**测试结果（山河志 · 战略地图）**
- 打开：dock 山河 → 城内布防图（默认页签）→ 切「山河志·十三州」→ D3 渲染正常（369 path / 61 郡面 + 州界 + 州郡城标签 + 「此身所在」打点）。
- 势力：默认「势力」层按当世归属着色（汉室32 / 孙策12 / 刘璋9 / 曹操2 / 刘表2 / 马腾2 / 董卓1 / 公孙度1，合计 61）；图例随 FAB 展开显示；「郡/版图/无」四层切换 active 态、面层显隐、图例联动全部正确；版图层 = 河/益/江/争 4 分区。
- 点选：城市点 → 详情面板（势力印章/州治/郡·州·规模/前往按钮）；州郡面 → 治理面信息；野外地点 → 关隘/副本（含等级要求）。
- 交互：滚轮/按钮缩放、拖拽平移、◎定位、81 城市点（含苦役营）全部正常，无 pageerror。

**修复：前往此城按钮断线（v20260912c 遗留）**
- 现象：点详情面板「前往此城 / 前往」按钮无任何反应（只重复 toast 详情，不传送）。
- 根因：`strategic-map.js` 的 go-btn 回调只调 `opts.onCityClick`（引擎侧绑定为 `placeInfo` 详情展示）；真正传送的 `goRoomOnMap`（体力-4·食物-1·饮水-1·时间+30分）从未接线。
- 修复：`strategic-map.js` 的 `selectCity`/`selectPlace` 按钮改为优先调用新增 `opts.onCityGo`（无则回退 onCityClick）；`engine.js initStrategicMapInGame` 正常模式注入 `onCityGo → goRoomOnMap`。
- 实测：点「前往此城」传送到蓟城：room camp_tz1→jicheng，体力100→96、食物/饮水各-1、时间+30 分钟，toast/存档正常。
- 说明：教学未完成时点前往会被「先把话说完」拦截，属教学期保护（合理）；图例 `display:none` 为 FAB 折叠态设计（展开即显示）。

**版本**：`constants.js` VERSION → `20260918a`；`index.html` `.tt-ver` → v20260918a、`strategic-map.js` 引用 `?v=20260918a`。已 commit + push（`kegaiisyou/sanguo-rpg`）。
