...
| **山河志·空间地图** | ✅ 完成 | 「山河」弹窗由竖排列表改为**按出口方位铺展的网格空间地图**：节点按 `MAP_COORDS`(col/row) 定位（北在上、东在右）、连线表示出口通达关系、当前所处红点高亮且通达之路实线加深、悬停显示「〔通达〕方向·地名」；北邙巡山道/黑山寨/蓟城内部等密集区并排呈现方位 | `index.html`（`MAP_COORDS`、地图弹窗渲染、`.map-canvas/.map-pin` CSS） |
...
*最后更新：2026-07-24（历法系统[农历+公历双轨·12时辰] + 天候系统[8种·跨日随机，已精简描述] + NPC 右键式浮层菜单[光标处弹出·无描述] + 时间锚点订正[中平元年184→光和元年178，对齐 GAME_DESIGN] + 清理两个测试实体[主营木人桩 / 黑山寨演示NPC] 完成）*
