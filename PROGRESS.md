...
### 6.1 游戏状态（v0.2.0）
- 框架闭环自洽，**37 个场景房间**（颍川主营·颍川城·北邙山林·溪畔草庐·洛阳 + 蓟城诸坊[通衢/南门/北门/州牧/学宫/大市/酒楼/军营/练武场/茶肆/平民里/民居等] + 黑山寨等）
...
*最后更新：2026-07-25（**记录当前真实版本**：① 修正文档严重过时的「5 个场景房间」为真实 37 个房间（含蓟城诸坊与黑山寨）；② 设置面板改为标签页（画面/声音/游戏），底部 dock「更多」→「设置」，标题页同款但无调试台；③ 清风茶肆说书先生改讲先秦游侠/北地豪侠/汉末风云（去掉温酒斩华雄，因光和元年三国未起），游戏其余三国味保留；④ 视觉：随机不规则墨晕 + 底部行军尘烟；⑤ 历法/天候/山河志空间地图/NPC 浮层菜单/善恶双轨/装备系统/武学完整版/UI 卡片交互/战败封锁 等均已于此前落地。当前真实落地规模：**37 房间 / 24 对话对象 / 25 招式 / 28 敌人 / 8 事件 / 13 艺线 / 4 AI / 华雄 Boss**）*
