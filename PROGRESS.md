...
- 游戏其余三国味（标题「三国群侠传」、华雄 Boss 战「温酒斩华雄」）按设定保留，不动。
...
### 6.1 游戏状态（v0.2.0）
- 框架闭环自洽，**61 个场景房间**（颍川主营·颍川城·北邙山林·溪畔草庐·洛阳 + 蓟城诸坊[通衢/南门/北门/州牧/学宫/大市/酒楼/军营/练武场/茶肆/平民里/民居等] + 黑山寨 + 北疆边塞[渔阳12坊·白檀军屯6·燕山6]）
- 时间：年号「**光和元年**（公元 178）」，农历 + 公历双轨 + 12 时辰，状态栏常驻古历 + 天候
- 战斗：12 招式数据（9 招式 + 3 发力技巧，含绝技回马枪）+ 10 敌模板 + 4 AI + 华雄 Boss；半手动 + 自动战斗
...
*最后更新：2026-07-29（**记录当前真实版本**：① 修正文档严重过时的「5 个场景房间」为真实 61 个房间（含蓟城诸坊、黑山寨与新增北疆边塞）；② 设置面板改为标签页（画面/声音/游戏），底部 dock「更多」→「设置」，标题页同款但无调试台；③ 清风茶肆说书先生改讲先秦游侠/北地豪侠/汉末风云（去掉温酒斩华雄，因光和元年三国未起），游戏其余三国味保留；④ 视觉：随机不规则墨晕 + 底部行军尘烟；⑤ 历法/天候/山河志空间地图/NPC 浮层菜单/善恶双轨/装备系统/武学完整版/UI 卡片交互/战败封锁 等均已于此前落地；⑥ **新增北疆边塞地图**：按汉末真实地理校准——渔阳郡治在蓟城东北（今密云），渔阳以北为白檀军屯（滦水屯田戍堡，曹操北征乌桓所经）、再北即燕山山脉屏障（汉疆与塞外分界）；洛阳→蓟城→马市北出→渔阳→北门→白檀→燕山→古栈道出塞，构成完整边塞走廊；⑦ **玩家出生点暂设燕山·山口**（`yanshan_shankou`）。当前真实落地规模：**61 房间 / 43 对话对象 / 12 招式数据 / 10 敌模板 / 8 事件 / 13 艺线 / 4 AI / 华雄 Boss**）*
