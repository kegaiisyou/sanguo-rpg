...
*最后更新：2026-07-24（山河志空间地图增强：山河底色[中原/洛阳/幽州区域+网格+羊皮纸] + 距离刻度[1格≈60里·随缩放实时] + 默认以当前所处为中心 + 拖拽平移 + 滚轮/双指缩放 + 调试台「设置出生点」复用同一地图修复拥挤；**布局修正**：黑山寨原坐标落在蓟城内城带（col10·row0-2）被误认为在城里，已整体移至蓟城外东北山区（col13·row0-2）并加「黑山」山区标识，城外马市/流民棚保持郭外；历法/天候/NPC菜单/光和元年/清理测试实体 已完成）*
