...
bpIds.forEach(function (id) {
  const bp = BUILD[id];
  const isCity = !!bp.city;   // 城市营造蓝图：覆盖层形态，无 placed 工地对象
  if (!isCity) {
  ...
  // 城市蓝图专项：onTypes 非空、cellType 落在已知格型、labor ≥1
  if (isCity) {
    check(Array.isArray(bp.onTypes) && bp.onTypes.length >= 1, '[' + id + '] city 蓝图 onTypes 非空');
    check(typeof bp.cellType === 'string' && !!bp.cellType, '[' + id + '] city 蓝图有 cellType');
    check(bp.labor >= 1, '[' + id + '] city 蓝图 labor ≥1');
  }
  // key 唯一性（仅 placed 蓝图）
  if (!isCity) {
