...
(function (global) {
  ...
  TRIGGERS.push({
    ...
    steps: [
      ...
      // v20260911k 文案瘦身：开场不再一屏一坨 —— 每条只留一口气的长度，逐句落下
      { t: 'log', cls: 'sys', text: '铁链啷当，你被推进牢里。' },
      { t: 'log', cls: 'env', text: '牢门在身后轰然合拢，镣铐沉沉坠着手脚。尘土从门缝墙隙翻涌而起，栅外天光只剩一线。' },
      // v20260912g：删掉「怪哉，怪哉」那句；此处只留「某夜观天象……气数将尽」半句作引子，
      //   剩下的话（你这新来的变数多到看不清）留到玩家当面搭话时由周听涛自己说出口 ——
      //   开场不替他讲完，既少一屏字，也把「找他说话」变成一个自然的动力。
      { t: 'log', cls: 'npc', text: '栅里忽地响起一个拖长腔的嗓子：「某夜观天象，大汉四百年气数将尽……」' },
      { t: 'log', cls: 'sys', text: '尘土迷眼，你只觉栅外有人影晃动，却看不清是谁。〔点下方「牢门」可叩问牢头；人物列表里那人也能搭话。〕' },
      // v20260912f：系统介绍从「行囊」开始 —— 先让玩家看清自己身上剩什么，再谈别的。
      //   此刻只放行「行囊」一个页签，其余（任务/角色/队伍/山河/设置）一律还藏着，
      //   等真正提到时再逐个亮出来（unlockDock，见 core/engine.js）。
      { t: 'log', cls: 'env', text: '你低头看自己：一身赭色囚服，脚踝上一副生铁镣，走一步便哗啦一响——这差不多就是你如今全部的家当了。' },
      { t: 'unlockDock', key: 'pack' },
      { t: 'log', cls: 'order', text: '〔行囊〕点下方「🎒 行囊」——身上穿的囚服、脚上的镣铐都在里头；往后捡到、穿上什么，也都在此处看。' },
  ...
  TRIGGERS.push({
    ...
    steps: [
      ...
      { t: 'npcTalk', npc: 'zhoutingtao',
        prompt: '尘埃稍落，才看清栅里那人：蓬头垢面，满身旧伤，眼神却清亮。他拖长腔道：「某周听涛，天下数一数二的相士——」随即压低嗓子：「你这新来的，命数古怪，像一颗老夫看不透的变数。去营中寻些吃食来，老夫便替你寻一丝破解之法。这桩差事，你应是不应？」',
        asks: [
          { label: '〔应下〕好，我去寻些吃食来。',
