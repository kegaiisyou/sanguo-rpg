...
(function (global) {
  var ROOMS = {
    ...
    forest: {
      ...
      find: '细看：古木蔽日，小径荆棘丛生；林深处营寨火光隐约，时有呼喝之声——盗匪便盘踞于此。',
      npcs: [],
      items: ['林间散落的五铢钱'],
      actions: [
    ...
    yuyang_guomen: {
      ...
      exits: { '北': 'yuyang_nanmen', '南': 'ji_mashi', '西': 'yuyang_xishijie', '东': 'yuyang_dongshijie' },
      npcs: ['yuyang_tingli'],
      find: '细看：郭门两侧揭帖写满入城税则与宵禁时刻，墙根戍卒就着风沙啃硬饼。南去蓟城大道尘土飞扬，北入城去便是南门；西首兵营旌旗隐约，东边市声渐起——渔阳便是个北疆门户的模样。',
      items: [],
      actions: [
    ...
    yuyang_nanmen: {
      ...
      npcs: ['yuyang_shoumen'],
      find: '细看：南门砖石厚实，门额「渔阳」二字苍劲；守门戍卒甲胄沾尘，门内通衢大街车马往来如织。',
      items: [],
      actions: [
    ...
    yuyang_tongqu: {
      ...
      npcs: ['yuyang_shuoshu'],
      find: '细看：青石主道车辙深陷，槐柳稀疏边风硬；北去北门出边塞，南出郭门去蓟城，东市街通大市，西市街达县衙与军营。',
      items: [],
      actions: []
    },
    yuyang_beimen: {
    ...
    yuyang_xishijie: {
      ...
      npcs: [],
      find: '细看：西市街北首县衙朱榜高悬，西去军营旌旗猎猎，南接驿馆；偶有巡街武卒踏着冻土往来。',
      items: [],
      actions: []
    },
    yuyang_junya: {
      ...
      npcs: ['yuyang_ling'],
      find: '细看：县衙照壁斑驳，堂上县令正批阅边备簿册；墙角枷锁刑杖林立，差役押着一名醉汉走过。',
      items: [],
      actions: []
    },
    yuyang_junying: {
      ...
      npcs: ['yuyang_jiang'],
      find: '细看：旌旗猎猎，渔阳边军操演阵法，马蹄踏地如闷雷；营帐间兵器架、草料堆林立，皮革铁锈之气弥漫。',
      items: [],
      actions: [
    ...
    yuyang_yiguan: {
      ...
      exits: { '南': 'yuyang_junying' },
      npcs: ['yuyang_yicheng'],
      find: '细看：传舍驿馆中驿丞登记文书，墙上悬各郡悬赏与北疆军情；后院马厩备有驿马，卧房虽简却整洁。',
      items: [],
      actions: [
    ...
    yuyang_dongshijie: {
      ...
      npcs: [],
      find: '细看：东市街酒楼茶肆林立，胡商汉贾摩肩接踵；北首「望蓟楼」帘招高挑，南面茶肆炉香袅袅，东去便是大市。',
      items: [],
      actions: []
    },
    yuyang_dashi: {
      ...
      npcs: ['yuyang_shang'],
      find: '细看：市楼高耸，旗鼓号令开闭；东首皮货马具，西首茶盐布庄，行商与胡贾比划议价，北疆特产堆积如山。',
      items: [],
      actions: [
    ...
    yuyang_jiulou: {
      ...
      exits: { '南': 'yuyang_dongshijie' },
      npcs: ['yuyang_jiulou_zhanggui'],
      find: '细看：望蓟楼内觥筹交错，跑堂穿梭如蝶；二楼雅座有商贾低议军情，不妨上去听个一二。',
      items: [],
      actions: [
    ...
    yuyang_chasi: {
      ...
      exits: { '北': 'yuyang_dongshijie' },
      npcs: ['yuyang_shuosheng'],
      find: '细看：听涛茶肆炉香袅袅，说书人正讲到飞将军夜引弓处，满堂喝彩；邻桌老者似见多识广，或可知北山匪情。',
      items: [],
      actions: [
    ...
    baitan_yingmen: {
      ...
      npcs: [],
      find: '细看：营门鹿角夹道，残碑「白檀」字迹斑驳；屯卒验符传出入，门内北去屯寨，西畔滦水萦回，山影即燕山。',
      items: [],
      actions: [
    ...
    baitan_tun: {
      ...
      find: '细看：木栅为垣，望楼高耸；田卒披甲持耒，半农半兵。寨西屯田、寨东马苑，北口通燕山；点将台上屯尉正考校武艺。',
      items: [],
      actions: [
    ...
    baitan_tian: {
      ...
      npcs: ['baitan_tianzu'],
      find: '细看：阡陌沿滦水展开，田卒凿渠垦殖；界碑「屯田」旁茅亭歇晌，老卒扶犁喘息，新垦多砾石、收成薄。',
      items: [],
      actions: [
    ...
    baitan_ma: {
      ...
      npcs: [],
      find: '细看：马苑围栏连绵，北地良驹就食；马卒刷鬃添草，性烈者萧萧嘶鸣。西通屯寨，北接哨垒。',
      items: [],
      actions: []
    },
    baitan_cang: {
      ...
      exits: { '南': 'baitan_tian' },
      npcs: [],
      find: '细看：圆廪贮粟，草苫封顶，卫卒持戈巡守；「军资重地」木牌森然。廪中粮秣乃军屯命脉。',
      items: [],
      actions: []
    },
    baitan_liao: {
      ...
      exits: { '南': 'baitan_ma' },
      npcs: ['baitan_guren'],
      find: '细看：土垒高筑，烽燧相望；哨卒举烽北通燕山、南通渔阳。垒上遥见雪岭，鲜卑游骑时现山影外。',
      items: [],
      actions: [
    ...
    yanshan_feng: {
      ...
      npcs: ['yanshan_yinshi'],
      find: '细看：危峰巨石嵯峨，绝顶俯瞰滦水如带、屯堡星罗；北望塞外天低野旷。南回山口，北去雪岭；峰侧隐者结庐。',
      items: [],
      actions: [
    ...
    yanshan_jian: {
      ...
      exits: { '东': 'yanshan_shankou' },
      npcs: [],
      find: '细看：山裂为涧，碎冰乱石壅塞，细流呜咽；人迹罕至，绝地也。东回山口，涧底兽骨断戟昭示酷寒刀兵。',
      items: [],
      actions: []
    },
    yanshan_lindao: {
      ...
      exits: { '南': 'yanshan_shankou', '北': 'yanshan_zhandao' },
      npcs: ['yanshan_liehu'],
      find: '细看：密林蔽天，腐叶没膝，猎径屈曲向北通古栈道；兽蹄印与鲜卑弃物杂陈。南回山口，北接栈道。',
      items: [],
      actions: [
    ...
    yanshan_zhandao: {
      ...
      desc: [
        ...
        '南接林道，北向塞外——过此便非汉家疆界，鲜卑、乌桓游牧其上。山风烈烈，似闻金鼓远来。'
      ],
      exits: { '南': 'yanshan_lindao' },
      npcs: ['yanshan_xiangdao'],
      find: '细看：堑山堙谷旧道，乃北出塞外之古径；木栈朽蚀，石阶磨平。南接林道，北向塞外——过此即非汉家疆界。',
      items: [],
      actions: [
    ...
    yanshan_xueling: {
      ...
      npcs: [],
      find: '细看：岭上经年积雪，朔风如刀；北眺草海乃鲜卑牧地，南顾层峦掩白檀屯与渔阳。南回峰峦，汉疆极北。',
      items: [],
      actions: [
    ...
    ji_chasi: {
      ...
      npcs: ['shuosheng'],
      find: '细看：清风茶肆炉香袅袅，说书人正讲到朱亥袖椎处，满堂喝彩；邻桌老者似见多识广，或可知北山匪情。',
      items: [],
      actions: [
        { id: 'rest', label: '茶肆歇脚', group: '行动', tip: '品茗小坐，气血内力尽复' }
      ]
    ...
    ys_entrance: {
      ...
      desc: [
        '燕山北麓，风如刀削。这座边荒小村的土墙塌了半边，火光里夹杂着妇孺惊呼与胡笳嘶鸣——北方部族的一支游骑，趁夜扑了上来。',
        '你（一个本不该在此世的魂灵）蜷在谷草堆里尚不及回神，一只枯手已将自己拽进灶膛阴影。「别出声，小叫花。」昏暗中，一个衣衫褴褛的老乞丐按住了你。'
      ],
      exits: { '北': 'ys_north' },
      npcs: ['beggar_old'],
      find: '细看：村口土墙塌处余烬未熄，老乞丐蹲在灶影里，浑浊眼里却精光一闪，似在掂量你这「死而复生」的野小子。',
      items: [], actions: []
    },
    ys_north: {
      id: 'ys_north', name: '燕山·北巷',
      desc: [
        '北巷窄仄，两面土墙夹着一线夜空。远处马蹄声渐密，老乞丐拽着你贴墙疾走：「快，莫回头。」'
      ],
      exits: { '东北': 'ys_ne', '南': 'ys_entrance' },
      npcs: [], items: [],
      find: '细看：巷口血迹拖出一串，往东北猎棚方向去了。',
      actions: []
    },
    ys_ne: {
      id: 'ys_ne', name: '燕山·东北猎棚',
      desc: [
        '猎棚空着，兽夹与弓弩挂在壁上。老乞丐从棚后摸出半块硬饼塞你手里：「先填肚子，待会儿有硬仗。」'
      ],
      exits: { '东': 'ys_east', '西南': 'ys_north' },
      npcs: [], items: [],
      find: '细看：棚后小径往东可通溪桥，西南方回北巷。',
      actions: []
    },
    ys_east: {
      id: 'ys_east', name: '燕山·东溪桥',
      desc: [
        '溪桥木板朽了一半，水声哗哗。刚踏上桥，林子里窜出三名太平道流民，黄巾缠臂，叫嚣着要夺路而走。老乞丐喝你「护住要害」，自己迎了上去。'
      ],
      exits: { '东南': 'ys_se', '西': 'ys_ne' },
      npcs: [], items: [],
      find: '细看：桥面留着凌乱脚印与几点血迹；西北回猎棚，南去田垄。',
      actions: []
    },
    ys_se: {
      id: 'ys_se', name: '燕山·东南田垄',
      desc: [
        '田垄收割过半，秸秆在风里沙沙作响。老乞丐替你按住流血的伤口：「小子，先看看你自己伤成什么样——点右下角『角色』，气血内力尽在里头。」'
      ],
      exits: { '南': 'ys_south', '西北': 'ys_east' },
      npcs: [], items: [],
      find: '细看：田垄尽头南去磨坊，西北回溪桥。',
      actions: []
    },
    ys_south: {
      id: 'ys_south', name: '燕山·南磨坊',
      desc: [
        '磨坊石碾停转，梁上挂着风干的小鱼。老乞丐道：「药给了，会用不？点『行囊』，把那瓶行军散用上，气血便回得来。」'
      ],
      exits: { '西南': 'ys_sw', '北': 'ys_se' },
      npcs: [], items: [],
      find: '细看：磨坊西南去谷场，北回田垄。',
      actions: []
    },
    ys_sw: {
      id: 'ys_sw', name: '燕山·西南谷场',
      desc: [
        '谷场草垛如山，远处火把游移——乌桓的游骑已嗅到踪迹。老乞丐神色一凛：「教你的拳脚，方才那套，记牢了。」'
      ],
      exits: { '西': 'ys_west', '东北': 'ys_south' },
      npcs: [], items: [],
      find: '细看：谷场西去柴院，东北回磨坊。',
      actions: []
    },
    ys_west: {
      ...
      exits: { '西北': 'ys_nw', '东': 'ys_sw' },
      npcs: [], items: [],
      find: '细看：柴院西北去哨所，东回谷场。',
      actions: []
    },
    ys_nw: {
      id: 'ys_nw', name: '燕山·西北哨所',
      desc: [
        '哨所塌了半边，木栅外夜色沉沉，北望便是燕山口的乱石径。老乞丐拍拍你肩：「去罢，小叫花。往后江湖路远，莫忘今夜这套拳。」'
      ],
      exits: { '北': 'yanshan_shankou', '东南': 'ys_west' },
      npcs: [], items: [],
      find: '细看：哨所北通便燕山山口（开放世界），东南回柴院。',
      actions: []
    }
  };
